...
export default function PublishingForm({ contents, devices, groups, tenantId, post, onPublished }: {
    ...
    try {
      ...
      const created = await post<Playlist>(`/api/v1/tenants/${tenantId}/playlists`, {
        ...
        items: [{
          ...
          captionText: String(values.get('captionText') ?? '').trim() || null,
        }],
      })
      ...
      await post(`/api/v1/tenants/${tenantId}/${targetPath}/assignments`, {
        ...
        priority: 0,
        overrideEqualPriority: true,
        startsAtUtc: null,
        endsAtUtc: null,
        presentationTimeZone: 'UTC',
      })
      form.reset(); setSelectedVersionId(''); setCaptionText('')
  ...
  return <form className="stack-form publishing-form" onSubmit={(event) => void submit(event)}>
    ...
    <label>Contenu<select name="content" required value={selectedVersionId} onChange={(event) => { setSelectedVersionId(event.target.value); setCaptionText('') }}><option value="">Sélectionner</option>{contents.map((content) => <option key={content.id} value={content.latestVersion?.id}>{content.title}</option>)}</select></label>
    <CompactContentPreview captionText={captionText} content={selectedContent} tenantId={tenantId} />
    {selectedContent && selectedContent.mediaKind.toLowerCase() !== 'plaintext' && <label>Texte superposé au contenu (facultatif)<textarea name="captionText" maxLength={1000} rows={3} value={captionText} placeholder="Écrivez ici le texte à afficher sur l’image ou la vidéo…" onChange={(event) => setCaptionText(event.target.value)} /></label>}
...
function CompactContentPreview({ captionText, content, tenantId }: { captionText: string; content: ContentItem | null; tenantId: string }) {
  ...
  return <div className={`publishing-preview ${content ? '' : 'publishing-preview-empty'}`} aria-live="polite">
    ...
    {content && <>
      ...
      <div className="publishing-preview-stage">
        ...
        {!error && ['mp4', 'jpeg', 'png', 'webp'].includes(mediaKind) && <MediaStage alt={`Aperçu de ${content.title}`} captionText={captionText.trim()} mediaKind={mediaKind} src={previewUrl} />}
        {!error && mediaKind === 'plaintext' && (linkedUrl
          ...
          : <p>{text || 'Chargement de l’aperçu…'}</p>)}
      </div>
    </>}
  </div>
}
