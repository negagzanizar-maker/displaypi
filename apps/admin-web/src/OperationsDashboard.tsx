...
import { createLinkedContent, readLinkedContent } from './operations/linkedContent'
...
function OperationsDashboard({ session, post, view }: { session: Session; post: PostJson; view: TenantView }) {
  ...
  const load = useCallback(async () => {
    ...
    setDevices(nextDevices as Device[]); setSelectedDevice((current) => current ? (nextDevices as Device[]).find((device) => device.id === current.id) ?? null : null); setLicenses(nextLicenses as License[]); setContents(nextContents as ContentItem[]); setPlaylists(nextPlaylists as Playlist[]); setDeviceGroups(nextDeviceGroups as DeviceGroup[])
    if (canAdminister) {
  ...
  useEffect(() => {
    if (view !== 'dashboard' && view !== 'devices') return
    const refreshPlayback = () => {
      void loadList<Device>(`/api/v1/tenants/${tenantId}/devices`)
        .then((nextDevices) => { setDevices(nextDevices); setSelectedDevice((current) => current ? nextDevices.find((device) => device.id === current.id) ?? null : null); setRefreshedAt(new Date().toISOString()) })
  ...
  const uploadLink = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); const form = event.currentTarget; const values = new FormData(form)
    await mutate(async () => {
      const body = new FormData()
      body.set('Title', String(values.get('Title') ?? '').trim())
      body.set('File', new File([createLinkedContent(String(values.get('Url') ?? ''))], 'linked-page.txt', { type: 'text/plain' }))
      const response = await fetch(`/api/v1/tenants/${tenantId}/contents`, { method: 'POST', credentials: 'same-origin', headers: { Accept: 'application/json', 'X-CSRF-TOKEN': session.csrfToken }, body })
      if (!response.ok) throw new Error(await readApiProblem(response)); form.reset()
    }, 'Lien contrôlé et ajouté aux contenus.')
  }
  ...
  return <>
    ...
    {view === 'devices' && <section className="data-section" id="view-devices" aria-labelledby="devices-title">
      ...
      <div className="device-grid">{devices.map((device) => { const licensed = device.licenseState === 'active'; const playback = device.playback; const isPlaying = playback?.playerState === 'ready' && playback.contentId && playback.contentVersionId && playback.mediaKind; const previewUrl = isPlaying ? `/api/v1/tenants/${tenantId}/contents/${playback.contentId}/versions/${playback.contentVersionId}/preview` : ''; return <button className={`device-card ${licensed ? 'device-card-licensed' : 'device-card-unlicensed'}`} key={device.id} onClick={() => setSelectedDevice(device)} type="button"><span className="device-card-status"><span aria-hidden="true" />{licensed ? 'Licence active' : 'Licence inactive'}</span><strong>{device.displayName}</strong>{isPlaying ? <DevicePlaybackPreview device={device} previewUrl={previewUrl} /> : <span className="device-current-empty">{playback?.playerState === 'synchronizing' ? 'Synchronisation…' : 'Rien en cours de lecture'}</span>}<span className="device-card-state"><Badge value={device.state} /><Badge value={device.health} /></span><span className="device-card-action">Voir les détails <span aria-hidden="true">→</span></span></button> })}</div>
    ...
    {view === 'content' && <section className="data-section" id="view-content" aria-labelledby="contents-title">
      <div className="section-heading"><div><p className="eyebrow">Médiathèque privée</p><h2 id="contents-title">Contenus</h2></div><p>JPEG, PNG, WebP, MP4, texte UTF-8 ou page web. Aucun fichier n’est public.</p></div>
      ...
      {canManageContent && contentSection === 'manage' && <form className="inline-form content-upload-form" onSubmit={(event) => void uploadLink(event)}><label>Titre du lien<input name="Title" maxLength={200} required /></label><label>Lien HTTP/HTTPS<input name="Url" type="url" placeholder="https://example.com" required /></label><button className="primary-button" disabled={busy} type="submit">Ajouter le lien</button><small className="form-help">La page doit autoriser son affichage intégré.</small></form>}
    ...
    {view === 'playlists' && <section className="data-section" id="view-playlists" aria-labelledby="playlists-title">
      ...
      {canManageContent && playlistSection === 'manage' && <div className="workflow-grid"><PublishingForm contents={approvedContents} devices={activeDevices} groups={deviceGroups} tenantId={tenantId} post={post} onPublished={load} /><PlaylistEditor contents={approvedContents} devices={activeDevices} groups={deviceGroups} tenantId={tenantId} post={post} onCreated={(created) => { setPlaylists((current) => [...current, created]); setNotice('Playlist enregistrée et publiée.') }} /></div>}
      {canManageContent && playlistSection === 'records' && <PlaylistManagement playlists={playlists} tenantId={tenantId} post={post} onChanged={load} />}
      {!canManageContent && <section className="collection-surface" aria-labelledby="saved-playlists-title"><div className="collection-heading"><div><p className="eyebrow">Catalogue</p><h3 id="saved-playlists-title">Playlists enregistrées</h3></div><span>{playlists.length}</span></div><div className="card-list">{playlists.map((playlist) => <article key={playlist.id}><div><strong>{playlist.name}</strong><small>{playlist.latestVersion?.itemCount ?? 0} élément(s)</small></div><Badge value={playlist.latestVersion?.publicationState ?? 'vide'} /></article>)}</div>{playlists.length === 0 && <p className="empty-state">Aucune playlist enregistrée.</p>}</section>}
    </section>}
...
function formatBytes(value: number | null | undefined) {
  ...
  return `${amount.toLocaleString('fr-FR', { maximumFractionDigits: unit === 0 ? 0 : 1 })} ${units[unit]}`
}
...
function DeviceDetails({ canRename, device, onClose, onRename }: { canRename: boolean; device: Device; onClose: () => void; onRename: (displayName: string) => void }) {
  ...
  return <div className="device-detail-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="device-detail" role="dialog" aria-modal="true" aria-labelledby="device-detail-title"><div className="device-detail-heading"><div><p className="eyebrow">Détails de l’appareil</p><h2 id="device-detail-title">{device.displayName}</h2></div><button className="text-button" onClick={onClose} type="button">Fermer</button></div><div className="device-detail-body"><section className="device-detail-section"><h3>Identité et état</h3><div className="device-detail-grid"><Detail label="Nom d’affichage" value={device.displayName} /><Detail label="Nom système" value={device.hostname} code /><Detail label="Numéro de série" value={device.serialNumber} code /><Detail label="État" value={<Badge value={device.state} />} /><Detail label="Santé" value={<Badge value={device.health} />} /><Detail label="Licence" value={<Badge value={device.licenseState ?? 'aucune'} />} /></div></section><section className="device-detail-section"><h3>Système Raspberry Pi</h3><div className="device-detail-grid"><Detail label="Système d’exploitation" value={device.osDescription} /><Detail label="Architecture" value={device.architecture} code /><Detail label="Version de l’agent" value={device.agentVersion} code /><Detail label="Version du lecteur" value={device.playerVersion} code /><Detail label="Capacité du disque" value={formatBytes(device.diskCapacityBytes)} /><Detail label="Espace disque libre" value={formatBytes(device.freeDiskBytes)} /></div></section><section className="device-detail-section"><h3>Réseau</h3><div className="device-detail-grid"><div className="device-detail-field"><span>Adresses IP locales</span><NetworkAddresses device={device} kind="ip" /></div><div className="device-detail-field"><span>Adresses MAC</span><NetworkAddresses device={device} kind="mac" /></div><Detail label="IP vue par le serveur" value={device.serverObservedIp} code /></div></section><section className="device-detail-section"><h3>Lecture et suivi</h3><div className="device-detail-grid"><Detail label="Expiration de licence" value={formatDate(device.licenseExpiresAtUtc)} /><Detail label="Dernière activité" value={formatDate(device.lastSeenUtc ?? null)} /><Detail label="Manifest publié" value={device.appliedManifestVersion ?? null} /><Detail label="Santé de lecture" value={device.playbackHealthCode} code /><Detail label="Identifiant interne" value={device.id} code /></div></section>{canRename && <form className="device-rename-form" onSubmit={(event) => { event.preventDefault(); const value = displayName.trim(); if (value && value !== device.displayName) onRename(value) }}><label>Modifier le nom d’affichage<input value={displayName} maxLength={160} onChange={(event) => setDisplayName(event.target.value)} required /></label><button className="primary-button" disabled={!displayName.trim() || displayName.trim() === device.displayName} type="submit">Enregistrer le nom</button></form>}</div></section></div>
}

function DevicePlaybackPreview({ device, previewUrl }: { device: Device; previewUrl: string }) {
  const playback = device.playback
  const [text, setText] = useState('')
  const mediaKind = playback?.mediaKind?.toLowerCase() ?? ''

  useEffect(() => {
    setText('')
    if (mediaKind !== 'plaintext') return
    const abort = new AbortController()
    void fetch(previewUrl, { credentials: 'same-origin', cache: 'no-store', signal: abort.signal })
      .then((response) => response.ok ? response.text() : Promise.reject())
      .then(setText)
      .catch(() => { if (!abort.signal.aborted) setText('Aperçu indisponible') })
    return () => abort.abort()
  ...
  return mediaKind === 'plaintext'
    ? linkedUrl
      ? <iframe className="device-current-link" src={linkedUrl} title={`Contenu diffusé sur ${device.displayName}`} sandbox="allow-forms allow-scripts allow-same-origin" referrerPolicy="no-referrer" />
      : <span className="device-current-text">{text || 'Chargement…'}</span>
    : <MediaStage alt={`Contenu diffusé sur ${device.displayName}`} captionText={playback?.captionText} className="device-current-preview" mediaKind={mediaKind} src={previewUrl} />
}
